(function (window){
    ...
    function transformForItemView(view, offset)
    {
        ...
        var spacing = 1;
        var arc = Math.PI * 2.0;
        var radius = 70; //Math.max(_itemWidth * spacing / 4.0, _itemWidth * spacing / 4.0 / Math.tan(arc/4.0/count));
        var angle = offset / count * arc;
        
        var x = radius * Math.sin(angle) * 3,
//             z = (radius * Math.cos(angle) - radius) *1,
			y = - Math.cos(angle) * 20, 
			z = radius * Math.cos(angle),
            s = (0.1 * Math.cos(angle) + 1) * 0.4;
        
        view.style.webkitTransform = " translate3d(" + x.toFixed(8) + "px, " +  y + "px, " + z.toFixed(8)  + "px) scale("+ s.toFixed(8) +")";
...
//         }
    ...
    {
        scrollByOffset(-0.2+minScrollDistanceFromOffset(_scrollOffset, offset), duration);
    }
